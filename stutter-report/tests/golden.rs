...
fn golden_minimal_report() {
    let input_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/minimal/input.json");
    let expected_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/minimal/expected.txt");
