...
OUT_DIR="${OUT_DIR:-${REPO_ROOT}/target/smoke/autotune-suggest}"
...
PROFILES="${PROFILES:-${REPO_ROOT}/examples/profiles/common-game-layouts.toml}"

mkdir -p "${OUT_DIR}"
rm -f "${DECISION_LOG}" "${STDOUT_LOG}" "${STDERR_LOG}"

if [[ ! -f "${PROFILES}" ]]; then
    echo "profile file not found: ${PROFILES}" >&2
    exit 2
fi
...
test -s "${DECISION_LOG}"
grep -F '"mode":"suggest"' "${DECISION_LOG}" >/dev/null
grep -F '"decision":"noop"' "${DECISION_LOG}" >/dev/null
grep -E 'autotune profiles=|candidate|suggest' "${STDOUT_LOG}" >/dev/null

echo "autotune suggest smoke passed"
