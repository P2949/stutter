...
val_path = "stutter/src/autotune/planning/profile_candidates/validate.rs"

new_validate = """use crate::profiles::{Profile, ProfileRule};
...
impl GeneratedProfileInvariants {
    pub fn check(profile: &Profile, policy: &GeneratedCpuSetPolicy) -> Self {
        ...
        for rule in &profile.rules {
            if let Some(affinity) = rule.affinity.as_ref() {
                ...
                if rule_matches_compositor_or_gamescope(rule) && cpu_count < policy.min_compositor_cpus {
                    compositor_has_cpu = false;
                }
                
                if profile.name != "baseline-online" && rule_matches_background_or_helper_work(rule) {
    ...
    pub fn is_valid(&self) -> bool {
        self.render_has_cpu && self.compositor_has_cpu && self.background_capacity_ok && self.no_empty_masks
    }
...
pub(crate) fn validate_generated_rule_mask(
    ...
    if let Some(allowed) = &policy.allowed_cpus {
        if !affinity.is_subset_of(allowed) {
            return Err(format!(
...
with open(val_path, "w") as f:
    f.write(new_validate)

mod_path = "stutter/src/autotune/planning/profile_candidates/mod.rs"
with open(mod_path, "r") as f:
    mod_content = f.read()

# Add pub use validate::GeneratedProfileInvariants;
if "GeneratedProfileInvariants" not in mod_content:
    mod_content = "pub use validate::GeneratedProfileInvariants;\n" + mod_content
