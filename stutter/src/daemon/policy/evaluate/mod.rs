...
pub(in crate::daemon::policy) use reason::record_policy_rule;
use reason::{finish_policy_explanation, validate_policy_descriptor_shape};
use risk::medium_risk_system_scope_allowed;
use target::record_context_rules;

use super::{
    ActionDescriptor, DaemonMode, DaemonPolicy, DaemonPolicyContext, DaemonPolicyVerdict,
    HIGH_RISK_APPLY_IMPLEMENTED, PolicyIntent, PolicyRejection, RollbackRequirement,
    capability::action_kind_matches_family,
};
use crate::{
    actions::SafetyClass,
...
impl DaemonPolicy {
    pub fn provider_confidence_threshold(&self, descriptor: &ActionDescriptor) -> f32 {
        mode::provider_confidence_threshold(self, descriptor)
    }
    ...
    pub fn explain_action_with_context(
        ...
        if first_rejection.is_some() {
            return finish_policy_explanation(
                self,
                intent,
                descriptor,
                evaluated_rules,
    ...
    pub fn explain_action(
        ...
        if remote::remote_target_count_blocks_non_apply(
            self,
            &mut evaluated_rules,
            &mut first_rejection,
        ) {
        ...
        remote::record_remote_apply_gates(self, &mut evaluated_rules, &mut first_rejection);
        ...
        let medium_risk_system_scope_allowed = medium_risk_system_scope_allowed(self, descriptor);
        ...
        finish_policy_explanation(self, intent, descriptor, evaluated_rules, first_rejection)
    }
