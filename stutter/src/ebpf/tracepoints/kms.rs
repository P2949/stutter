...
pub(crate) struct KmsProviderTracepointOffsets {
    pub(crate) request_card_minor_offset: u32,
    pub(crate) request_crtc_offset: u32,
    pub(crate) request_pipe_offset: u32,
    pub(crate) done_card_minor_offset: u32,
    ...
    pub(crate) vblank_card_minor_offset: u32,
...
pub(crate) fn kms_provider_tracepoint_offsets(
    ...
    let request_card_minor = request.and_then(find_kms_card_minor_field);
    let request_crtc =
    ...
    let done_card_minor = done.and_then(find_kms_card_minor_field);
    let done_crtc =
    ...
    let vblank_card_minor = vblank.and_then(find_kms_card_minor_field);
    let vblank_crtc =
    ...
    Some(KmsProviderTracepointOffsets {
        request_card_minor_offset: request_card_minor.map(|field| field.offset).unwrap_or(0),
        ...
        done_card_minor_offset: done_card_minor.map(|field| field.offset).unwrap_or(0),
        ...
        vblank_card_minor_offset: vblank_card_minor.map(|field| field.offset).unwrap_or(0),
...
fn find_kms_card_minor_field(format: &DrmTracepointFormat) -> Option<&DrmTracepointField> {
    find_kms_field(format, &["card_minor", "minor", "card"])
}
