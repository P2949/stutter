...
mod tests {
    ...
    #[test]
    fn autotune_root_module_is_thin_facade() {
        let source = include_str!("../mod.rs");

        assert!(
            !source.contains("pub struct AutotuneCommandInput"),
            "AutotuneCommandInput must live in autotune::commands::live, not autotune/mod.rs"
        );
        assert!(
            !source.contains("pub async fn autotune_command"),
            "autotune_command must live in autotune::commands::live, not autotune/mod.rs"
        );
        assert!(
            !source.contains("fn runtime_config_for_command"),
            "runtime_config_for_command must live in autotune::commands::live, not autotune/mod.rs"
        );
        assert!(
            !source.contains("println!("),
            "autotune/mod.rs must not contain command rendering or printing"
        );
    }
