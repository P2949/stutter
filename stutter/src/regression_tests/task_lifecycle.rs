...
fn reused_tid_with_different_task_resets_stats_after_removal() {
    ...
    tasks::reactivate_or_reset_stats_inner(&mut stats_by_task, None, 42.into(), &new_task, 77);
    ...
    assert_eq!(stats.process_id().map(|pid| pid.as_u32()), Some(200));
...
fn active_same_tid_replacement_resets_stats_even_without_remove_add_diff() {
    ...
    let active_targets = BTreeMap::from([(42.into(), old_task.clone())]);
    let desired_tasks = BTreeMap::from([(42.into(), new_task.clone())]);
    ...
    let stats_by_task = BTreeMap::from([(42.into(), stats)]);
...
fn same_reused_tid_reactivates_without_clearing_stats() {
    ...
    tasks::reactivate_or_reset_stats_inner(&mut stats_by_task, None, 42.into(), &same_task, 77);
...
fn event_comm_updates_only_unknown_existing_name() {
    ...
    let stats_by_task = BTreeMap::from([(7.into(), metrics::TaskStats::new(7, "?".to_owned(), 0))]);
