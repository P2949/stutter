...
fn parses_daemon_reset_state_command() {
    let command =
        parse_daemon_command(["stutter", "daemon", "reset-state", "--dry-run", "--json"]).unwrap();
...
fn parses_daemon_soak_command() {
    ...
    assert_eq!(
        ...
        crate::daemon::testing::DaemonSoakProfile::ApplyLowRiskFake
    );
...
fn parses_daemon_resync_state_command() {
    let command =
        parse_daemon_command(["stutter", "daemon", "resync-state", "--dry-run", "--json"]).unwrap();
