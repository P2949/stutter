...
| Display topology | Which GPU is rendering, which GPU is scanning out, and what compositor/session metadata was visible? | `ProbeKey::DisplayTopology`, `display_topology.json`, `session.display_path`, and `--preset prime-display-path` are implemented. | Implemented as a low-overhead best-effort sysfs/procfs snapshot. Unknown topology or missing old-run artifacts are unavailable evidence, not proof of direct scanout. |
| GPU engine sampling | Was render, blitter, copy, or GFX activity present on the render or scanout GPU near frame outliers? | `ProbeKey::GpuEngineSampling`, `--gpu-engine-sampling`, `gpu_engine_samples.json`, session counters, validation, and report summary are implemented. | Implemented with hwmon-derived baseline sampling. It is activity evidence rather than exact per-client engine accounting. |
| DMABUF path tracking | Did format/modifier, cross-GPU import, or copy-required hints correlate with the display path? | `ProbeKey::DmaBufPathTracking`, `--dmabuf-tracking`, `--dmabuf-log`, `dmabuf_events.json`, session counters, validation, and report summary are implemented. | Implemented for cooperative NDJSON logs. Missing events are unavailable evidence; copy-required events are candidate attribution, not exact copy latency. |
| Direct scanout status | Did cooperative presentation or DMABUF hints indicate direct scanout, compositing, mixed state, or unknown? | `ProbeKey::DirectScanoutStatus`, `direct_scanout` report summary, `docs/DIRECT_SCANOUT_LOG.md`, and cooperative Wayland/DMABUF fields are implemented. No standalone raw artifact is emitted. | View-only derived summary. Missing direct-scanout hints are unavailable evidence, not proof of direct scanout success. |
