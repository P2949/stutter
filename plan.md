...
- [x] Patch 1.8/architecture final split: moved direct-print checks into `src/architecture_tests/direct_prints.rs`, unwrap/expect checks into `src/architecture_tests/unwrap_expect.rs`, and the remaining scanner/dependency boundary tests into their owning child modules; `architecture_tests.rs` is now a 26-line dispatch module.
- [x] Patch 1.8/regression tests: split `src/regression_tests.rs` into focused child modules under `src/regression_tests/` for support, task lifecycle, latency metrics, process model/snapshot, recording, reporting, serialization, streaming, and tune coverage; `regression_tests.rs` is now a 39-line dispatch module and its stale file-size/unwrap allowlist entries were removed.
- [x] Patch 1.8/test fixture builder: split `src/test_fixture_builder.rs` into `src/test_fixture_builder/{corpus,fixtures,metadata,model}.rs` plus focused fixture constructor modules; `test_fixture_builder.rs` is now a 31-line dispatch module and its stale oversized-file allowance was removed.
- [x] Patch 1.8/agent tests: split `src/agent/tests.rs` into focused modules under `src/agent/tests/` for shared support, daemon routes, remote policy, autotune handlers, remote request validation, security/auth, and capability responses; `agent/tests.rs` is now a 13-line dispatch module and stale file-size/unwrap allowances were removed.
- [x] Patch autotune/apply-low-risk model-executor: moved apply-low-risk plan/outcome models and the low-risk action executor path into `src/autotune/apply_low_risk/{model,executor}.rs`; `apply_low_risk.rs` dropped to 631 lines and its oversized-file allowance was lowered to 650 lines.
- [x] Patch autotune/apply-low-risk audit: moved audited rollback, candidate-to-action conversion, audited apply wrappers, history helper, and controller-journal hook helpers into `src/autotune/apply_low_risk/audit.rs`; `apply_low_risk.rs` dropped to 405 lines and its oversized-file allowance was lowered to 425 lines.
- [x] Patch autotune/apply-low-risk target: moved selector validation, procfs comm matching, and live tree validation into `src/autotune/apply_low_risk/target.rs`; `apply_low_risk.rs` dropped to 306 lines and its oversized-file allowance was lowered to 325 lines.
- [x] Patch autotune/apply-low-risk experiment: moved low-risk candidate selection, profile planning, experiment comparison/resolution wrappers, and readiness gates into `src/autotune/apply_low_risk/experiment.rs`; `apply_low_risk.rs` dropped to 182 lines and its oversized-file allowance was lowered to 200 lines.
- [x] Patch autotune/apply-low-risk command-facade: moved apply-low-risk command orchestration into `src/autotune/apply_low_risk/command.rs`; `apply_low_risk.rs` is now an 81-line facade and its oversized-file allowance was removed.
- [x] Patch actions/runner model: moved `AuditedActionResult`, `ActionHooks`, and `ActionRunPolicy` into `src/actions/runner/model.rs`; `runner.rs` dropped to 614 lines and its oversized-file allowance was lowered to 625 lines.
- [x] Patch actions/runner audit: moved runner audit event update/appending helpers into `src/actions/runner/audit.rs`; `runner.rs` dropped to 567 lines and its oversized-file allowance was lowered to 575 lines.
- [x] Patch actions/runner policy: moved daemon-policy explanation mapping into `src/actions/runner/policy.rs`; `runner.rs` dropped to 555 lines and its oversized-file allowance was lowered to 565 lines.
- [x] Patch actions/runner rollback: moved total-timeout detection plus timeout and after-apply-hook rollback helpers into `src/actions/runner/rollback.rs`; `runner.rs` dropped to 428 lines and its oversized-file allowance was lowered to 440 lines.
- [x] Patch actions/runner execute-facade: moved audited action execution entry points and phase orchestration into `src/actions/runner/execute.rs`; `runner.rs` is now a 28-line facade and its oversized-file allowance was removed.
- [x] Plan 1.1 partial-apply rollback safety: verified `PartialApplyError`/`ApplyResult`, runner partial-rollback handling, and transactional nice/ioprio/uclamp apply loops; added fake partial-apply runner coverage proving rollback, failed final outcome, and “partial rollback attempted” audit evidence.
- [x] Plan 1.2 shared target-loop transaction helper: expanded `src/actions/transaction.rs` with planned/applied records plus `plan`, `mark_applied`, `partial_token`, `rollback_applied`, and `apply_planned_loop`; converted nice/ioprio/uclamp to prebuild restore records before mutation and added generic plus action-token transaction tests.
- [x] Plan 2.1 identity-bearing restore records: added optional restore identity envelopes with top-level legacy `tid` compatibility for nice/ioprio/uclamp/cgroup records, preserved old identity-only and no-identity rollback token deserialization, copied process/thread identity into new rollback records, and added serde compatibility coverage.
- [x] Plan 2.2 identity verification before restore: expanded restore identity checks to verify task starttime and thread-group pid when identity is available, kept comm/exe as advisory warnings, wired compatible legacy handling and mismatch skips into nice/ioprio/uclamp/cgroup rollback paths, and added reused-TID coverage.
- [x] Plan 2.3 ESRCH-tolerant restore loops: added a shared restore-write classifier and summary, mapped ESRCH and missing `/proc/<tid>` to missing-task skips, made nice/ioprio/uclamp/cgroup rollback attempt all records before reporting real failures, and added dead-task, identity-mismatch, permission/invalid-value classification, and delayed-failure coverage.
- [x] Plan 3 remote autotune restore: wired `/autotune/restore` through the existing autotune emergency restore path, returned real no-op/restored/failed statuses with action and record-level restore counts, distinguished no-op daemon state from restored, and added unauthorized/no-op/active-rollback/failure response coverage.
- [x] Plan 4.1 recording task reaper: made finished recording reaping return explicit no-active/still-active/completed/failed status, verified record status clears completed runs, stale finished runs no longer block a new recording start, daemon status reaps join failures into faulted/degraded state, and still-active handles remain live.
- [x] Plan 4.2 autotune task reaper: made finished autotune reaping return explicit no-active/still-active/completed/failed status, verified autotune status clears completed sessions, stale finished sessions no longer block new starts, daemon status exposes join failures as faulted/degraded, and still-active handles remain live.
- [x] Plan 5.1 Unix socket connection cap: exposed the Unix socket connection cap through agent config and `--max-unix-connections`, wired `run_agent` into the capped socket server, added active/rejected/error connection logging, and covered extra idle-client rejection with a focused Unix socket server test.
- [x] Plan 5.2 Unix socket connection timeout: exposed the idle/read timeout through agent config and `--unix-connection-timeout-ms`, passed custom timeouts into the socket server, rejected zero timeout configuration, and covered idle close, active request success, and permit release after timeout.
- [x] Plan 6.1 cgroup rollback cpuset state: verified cgroup rollback tokens carry optional cpuset restore metadata with serde-default compatibility, apply snapshots original `cpuset.cpus`/`cpuset.mems` before writing, missing cpuset files are represented as `None`, and rollback restores only captured cpuset files.
- [x] Plan 6.2 transactional cgroup placement apply: routed cgroup apply through a rollback-aware helper, restored cpuset/task mutations before returning partial apply failures, kept partial rollback tokens for runner/emergency handling, and added injected write-failure coverage for `cpuset.mems` and second-task move failures.
- [x] Plan 7 active-config unknown degradation: made proposal-level and conflicting kept-action `ActiveConfigMatch::Unknown` produce `ActiveConfigUnknown` denials, kept unknown active experiments degraded/blocked, surfaced the unknown reason in deny messages and grouped planner summaries, and added focused planner coverage.
- [x] Plan 8 IRQ missing timestamp handling: removed the timestamp-`0` fallback, timestamped missing IRQ elapsed times from the current rolling-window anchor when available, dropped untimestamped IRQ events when no anchor exists, retained legacy pruning of `None`, and added bounded insertion/pruning coverage.
- [x] Plan 9 block I/O fallback collision handling: verified the fallback collision drop counter contract across common/eBPF/userspace, confirmed ambiguous fallback starts are dropped instead of attributed, counted collisions in drop totals while excluding them from non-block comparability, and added reporting/template coverage for the degradation warning.
- [x] Plan 10 sched-switch cheap-filter ordering: verified `sched_switch` keeps only the initial `next_pid` read before early returns and reads `prev_pid_raw`/`prev_state` only after wakeup-data and target filters, with a source-level regression test pinning that ordering.
- [x] Plan 11 privileged-worker socket readiness: kept worker startup readiness based on retrying `UnixStream::connect`, made the retry timing injectable for tests, and added stale-socket, connectable-listener, and clear-timeout coverage.
- [x] Plan 12 recording stop lock scope: verified `stop_record_handler` takes the active handle inside a short mutex scope and awaits `join` after the guard is dropped, with a regression test proving `active_run` remains lockable while shutdown is still pending.
- [x] Plan 13 foreground helper hardening: routed Sway/Hyprland/X11 helper execution through trusted helper resolution plus a sanitized command environment, verified malicious `PATH` entries are ignored, explicit absolute helper paths still work, unsafe env is stripped, and missing helpers degrade cleanly.
- [x] Plan 14 zero/invalid frametime handling: dropped zero, negative, and non-finite frame durations at ingestion, counted invalid frame drops, kept invalid timestamps driving pruning, and verified frame p99/max plus all-invalid windows cannot be improved by `0.0 ms` data.
- [x] Plan 15 enum-based autotune mode checks: kept apply-low-risk control flow on `DaemonMode`, extracted enum-based duration/runtime/message helpers, verified apply-low-risk runs without a fixed controller duration, applies candidate-window behavior, and returns the expected response message.
- [x] Plan 16 interval batch ordering hardening: made `push_intervals()` sort the merged interval stream and prune against the true latest timestamp, preventing older out-of-order batches from moving the window anchor backward, with coverage for older-after-newer batches, same-tick ordering, and empty no-op batches.
- [x] Plan 17 IRQ key layout documentation: confirmed the eBPF key already packs IRQ into the high 32 bits and CPU into the low 32 bits without a `65536` collision, kept behavior unchanged, and added a source-level regression pinning the comment and expression.
- [x] Phase 3.1 automatic display topology detection: completed topology artifact/loader plumbing, added cross-GPU/direct/unknown sysfs detection coverage, changed no-connector cases to unknown with warnings, and enriched session display-path metadata with topology-derived card/driver/session/compositor/confidence fields while preserving manual override fields.
- [x] Phase 3.2 derived direct-scanout summary: added `DirectScanoutSummary` to report JSON, derived yes/no/mixed/unknown status from cooperative Wayland presentation `zero_copy` and future direct-scanout/composited flags, kept missing evidence as unknown, and covered direct, composited, mixed, and empty-event cases.
- [x] Phase 3.3 stronger cross-GPU fence attribution: preserved DRM fence signal timestamps, added `CrossGpuFenceSummary`/candidate report JSON, enriched live KMS/fence records with configured card/driver hints, and classified high-confidence amdgpu-to-i915 display-side waits near frame outliers and KMS delays.
- [x] Phase 3.4 cooperative DMABUF format/modifier tracking: added `dmabuf_events.json` live ingestion from `--dmabuf-log`, wired `dmabuf_tracking` config/probe metadata/counters/session loading, added `DmaBufPathSummary` report JSON, and counted modifier mismatch, cross-GPU import, linear, scanout-capable, and copy-required candidates with parser and report coverage.
- [x] Phase 3.5 multi-GPU engine sampling: added `--gpu-engine-sampling`/probe wiring, `gpu_engine_samples.json` live emission from hwmon-derived `MultiGpuHwmonReader`, session counters/loading, and `GpuEngineActivitySummary` report JSON with i915 render/blitter and amdgpu GFX activity near frame outlier coverage.
- [x] Phase 3.6 display-path component diagnosis: added `DisplayPathDiagnosisSummary`/`DisplayPathComponent` report JSON, combined render/fence/KMS/Wayland/compositor component status, included direct-scanout/cross-GPU-fence/DMABUF/GPU-engine summaries, and rendered the diagnosis in text plus HTML model data.
- [x] Phase 3.7 stronger A/B compare: added `compare display-path --expect ... --strict`, carried the expectation through CLI/command input, expanded verdict/confidence/evidence/missing-evidence/component deltas, added topology EDID/mode comparability warnings, and covered the guided command parser.
- [x] Phase 3.8 single-run display-path suspicion score: derived a capped suspicion score with separate confidence from topology, fence, KMS, Wayland, direct-scanout, DMABUF, and GPU-engine evidence; surfaced evidence and missing-evidence lists in report JSON and text output.
- [x] Cross-cutting prime-display-path preset and probe registry: added the `prime-display-path` preset defaults, display-topology config/provenance/recorded-config wiring, `DisplayTopology` and derived `DirectScanoutStatus` probe registry entries, and probe activation coverage for the topology JSON artifact.
- [x] Cross-cutting display-path docs and artifact schema: added `docs/PRIME_DISPLAY_PATH.md`, `docs/DMABUF_PATH_LOG.md`, and `docs/DIRECT_SCANOUT_LOG.md`, and updated artifact schema, probe admission, DRM fence, Wayland presentation, and validation corpus docs with cautious candidate-attribution wording.
- [x] Phase 8 display-path validation corpus: added generated synthetic fixtures for direct GPU clean, UHD630 cross-GPU fence wait, UHD630 composited blitter, UHD630 KMS delay, Wayland zero-copy good, DMABUF modifier mismatch, and missing-evidence unknown; regenerated the corpus and covered display-path suspicion/direct-scanout/DMABUF/GPU-engine assertions.
- [x] Final validation gates: ran `cargo fmt --all`, `cargo build --all-targets`, `cargo clippy --all-targets -- -D warnings`, and `cargo test --all-targets`; the full test suite passed outside the sandbox because Unix socket tests require local socket binding.
